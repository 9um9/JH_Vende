...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Utils UI (atualizada para avatar)
  function appendMessage(text, from = 'bot') {
    ...
    if (from === 'bot') {
      ...
      avatar.alt = 'Assistente'; // acessibilidade
      wrap.appendChild(avatar);
      wrap.appendChild(bubble);
    } else {
      // usuário: sem avatar, apenas mensagem alinhada à direita
      wrap.appendChild(bubble);
    }
  ...
  // Remove listeners/implementações antigas de toggle/esc e substitui por lógica robusta
  // Gerencia estado do painel e do botão, previne propagação e fecha ao clicar fora
  let panelOpen = false;
  function setPanelOpen(open) {
    ...
    if (panelOpen) {
      ...
      // opcional: devolver foco ao botão
      aiToggleBtn.focus();
    }
  }

  // Toggle por clique no botão
  aiToggleBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    setPanelOpen(!panelOpen);
  });

  // Close button dentro do painel
  aiCloseBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    setPanelOpen(false);
  });

  // Evitar que cliques dentro do painel fechem-no
  ...
  // Tecla Escape fecha painel quando aberto
  ...
  const knowledgeBase = {
    ...
    open_box_example: {
      keywords: ['RTX,', 'Quais RTX tem?', 'tem RTX 2060?', 'tem RTX 2070?', 'tem RTX 2080?', 'tem RTX 3050?', 'tem RTX 3060?', 'tem RTX 3070?', 'tem RTX 3080?', 'tem RTX 3090?', 'tem RTX 4050?', 'tem RTX 4060?', 'tem RTX 4070?', 'tem RTX 4080?', 'tem RTX 4090?', 'tem RTX 5050?', 'tem RTX 5060?', 'tem RTX 5070?', 'tem RTX 5080?', 'tem RTX 5090?','RX', "rx 7600", "rx 7700", "rx 7800", "rx 7900", "rx 6600", "rx 6700", "rx 6800", "rx 6900"],
      response: 'No setor de placa de vídeo temos algumas RTX e RX tão boas quanto sua Geladeira, se não tiver, poderá mandar mensagem no zap (34) 99717-2028 para que possa consultar disponibilidade, sem nenhum problema! Atenderemos o mais breve possível.'
    }
  };
  ...
  function getBotReply(text) {
    ...
    const productSearch = searchProduct(text);
    if (productSearch) {
  ...
  // Eventos
  // Toggle: abre ou fecha o painel e esconde/mostra o botão conforme estado
  ...
  // Fechar com Escape quando painel aberto
  document.addEventListener('keydown', (e) => {
  ...
  aiSendBtn.addEventListener('click', () => {
    ...
    }, 450); // leve delay para simular "pensamento"
  });
  ...
  // Abrir painel se ?chat=1 na URL (opcional)
